...
## SLS·Virtual Vocal Coach 통합

활성 수업 화면에는 `SLS_EXERCISE`로 Mum → Guh → Go → Gee → Koo의 책 순서를 표시하고, 실제 레슨에서 별도로 사용되는 Goo는 비교·전이 도구로 구분했다. 각 단계의 역할은 책이 직접 설명한 생리적 기전이 아니라 코칭용 관찰 목표로 표시한다. `VIRTUAL_COACH_RULES`는 관찰 → 우선 시도 → 반응 판정 → 대안 비교 → 조건 조절 → 노래 전이 → 다음 수업 재현의 흐름을 데이터로 보존한다.
