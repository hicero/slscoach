...
export default function VocalDiagnostic({ onSelectType }: VocalDiagnosticProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 sm:p-6 md:p-8 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-150 dark:border-slate-700/50 pb-5 mb-5 gap-4">
        <div>
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200">
        ...
        {(history.length > 0 || diagnosticResult) && (
          <button
            onClick={handleReset}
            className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 transition-all font-medium self-start sm:self-auto shrink-0"
          >
            <RotateCcw size={12} />
            처음부터 다시
          </button>
        ...
        currentStep ? (
          <div>
            <div className="mb-5">
              ...
              <h3 className="text-sm md:text-base font-semibold text-slate-800 dark:text-slate-200 leading-snug mt-1">
            ...
            <div className="space-y-3">
              {currentStep.options.map((option, idx) => (
                <button
                   ...
                   className="w-full text-left p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-teal-500 hover:bg-teal-50/20 active:bg-teal-50/55 transition-all flex items-center justify-between group"
                >
                  <span className="text-xs md:text-sm text-slate-700 dark:text-slate-300 font-normal group-hover:text-teal-900 transition-colors">
                     <TermTooltip text={option.text} />
                  </span>
                  <div className="bg-slate-50 dark:bg-slate-800/40 group-hover:bg-teal-500 text-slate-400 group-hover:text-white p-1 rounded-lg border border-slate-100 dark:border-slate-800 group-hover:border-teal-400 transition-all ml-4 shrink-0">
          ...
          <div className="text-center py-8">
            <p className="text-sm text-slate-500 dark:text-slate-400">진단 진행 중 오류가 발생했습니다. 처음 단계로 이동합니다.</p>
        ...
        <div className="bg-gradient-to-br from-teal-50/50 via-emerald-50/30 to-transparent border border-teal-100 rounded-2xl p-6 md:p-8 animate-fade-in">
          <div className="flex items-start gap-4 mb-5">
            ...
            <div>
              ...
              <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-slate-100 tracking-tight mt-0.5">
          ...
          <div className="space-y-4 mb-5">
            <div>
              <h5 className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold mb-2">판별된 주요 보컬 특징</h5>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {diagnosticResult.characteristics.map((char, index) => (
                  <li key={index} className="bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 p-3 rounded-xl text-xs text-slate-700 dark:text-slate-300 flex items-start gap-2">
                    <span className="flex-shrink-0 w-4 h-4 bg-teal-100 text-teal-700 dark:text-teal-400 rounded-full flex items-center justify-center font-bold text-[9px]">
                      {index + 1}
                    </span>
            ...
            <div className="border-t border-slate-150 dark:border-slate-700/50 pt-3.5 flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {diagnosticResult.avoidTools.map(t => (
                    <span key={t} className="text-[10px] font-semibold bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 px-2 py-0.5 rounded border border-rose-200">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            <button
              onClick={handleReset}
              className="px-5 py-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-800/40 hover:border-slate-350 text-slate-600 dark:text-slate-400 font-semibold justify-center items-center flex gap-1.5 text-xs transition-all"
            >
              <RotateCcw size={12} />
              다시 진단
            </button>
